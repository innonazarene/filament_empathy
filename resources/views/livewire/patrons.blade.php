<div class="bg-gray-900">
    <div class="p-4 w-full flex justify-center">
        <p class="font-bold text-lg text-white border-b-2 border-b-white">
            PATRONS / SPONSORS
        </p>
    </div>
    <div class="p-4 grid md:grid-cols-3 grid-cols-1 gap-5 ">
        @forelse ($patrons as $item)
            <div class="container max-w-lg bg-white rounded dark:bg-gray-800 shadow-lg transform duration-200 easy-in-out md:m-12">
                <div class="flex justify-center px-5 -mt-12 mb-5">
                    <span class="block relative h-32 w-32">
                        @if($item->patron_images)
                            <img alt="Photo by aldi sigun on Unsplash"
                                src="{{ url('/storage/'.$item->patron_images[0]) }}"
                                class="mx-auto object-cover rounded-full h-24 w-24 bg-white p-1" />
                        @endif
                    </span>
                </div>
                <div class="px-7 mb-8">
                    <h2 class="text-3xl font-bold text-green-800 dark:text-gray-300">{{ $item->patron_name }}</h2>
                    <p class="mt-2 text-gray-600 dark:text-gray-300">{{ $item->patron_description }}</p>
                    ...
                    @endif
                    @if($item->penterest_link)
                        <button
                            class="p-1 sm:p-2  inline-flex items-center hover:text-gray-30 text-gray-400 hover:text-gray-300">
                            <svg class="w-7 h-7 fill-current" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path
